/* ============================================================
   ...
   Base: aula-01/aula01.js (mesmo tema e recursos)
   ============================================================ */
(function () {
  ...
  window.openPresentation = function () {
    ...
    presWin = window.open(url, 'ifpi_informatica_aula02');
    if (!presWin || presWin.closed) {
      var f = (location.pathname.split('/').pop() || 'aula02.html') + '#apresentacao';
  ...
  window.toggleProjetor = function () {
    ...
    setProjetor(on);
    try { localStorage.setItem('aula02-tema', on ? 'projetor' : 'leitura'); } catch (e) {}
    if (!isPres && presWin && !presWin.closed) { try { presWin.postMessage({ type: 'tema', projetor: on }, '*'); } catch (e) {} }
  };

  (function restoreTema() {
    var saved = null;
    try { saved = localStorage.getItem('aula02-tema'); } catch (e) {}
