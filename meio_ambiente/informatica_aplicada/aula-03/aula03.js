/* ============================================================
   Informática Aplicada · Aula 03 — Hardware e Software
   IFPI Campus Barras · passador de slides responsivo
   Base: aula-02/aula02.js (mesmo tema e recursos)
   ============================================================ */
(function () {
  ...
  window.openPresentation = function () {
    ...
    presWin = window.open(url, 'ifpi_informatica_aula03');
    if (!presWin || presWin.closed) {
      var f = (location.pathname.split('/').pop() || 'aula03.html') + '#apresentacao';
  ...
  window.toggleProjetor = function () {
    ...
    setProjetor(on);
    try { localStorage.setItem('aula03-tema', on ? 'projetor' : 'leitura'); } catch (e) {}
    if (!isPres && presWin && !presWin.closed) { try { presWin.postMessage({ type: 'tema', projetor: on }, '*'); } catch (e) {} }
  };

  (function restoreTema() {
    var saved = null;
    try { saved = localStorage.getItem('aula03-tema'); } catch (e) {}
  ...
  document.querySelectorAll('.comp .bege .foto, .bege .amp .foto').forEach(function (img) {
